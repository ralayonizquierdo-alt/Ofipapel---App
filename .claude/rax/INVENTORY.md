...
| Creative Lab | Módulo de investigación y perfeccionamiento de calidad visual dentro de Creative Engine — 9 bibliotecas atómicas, Biblioteca de Referencias (combina sin copiar, escalable a decenas de miles, con importador real desde campañas propias aprobadas — `reference-library/import-from-campaign.js`), Generador de Conceptos (8-12 por campaña) con filtro "Director de Arte Senior", Moodboard textual, Prompt Composer Cinematográfico (13 bloques, campaña no producto), Concept Score en dos capas, **Art Direction Engine** (elige entre 15 patrones editoriales premium — Hero Product, Apple Style, Nike Style, Muji Style, Luxury Minimal...— decide qué elementos sobran y qué iconos usar ANTES de calcular el grid), Layout Intelligence/Composition Engine (calcula grid/jerarquía/tamaños relativos/márgenes/espacios en blanco/equilibrio siguiendo el patrón elegido y los puntúa ANTES de renderizar — sin coordenadas fijas), **Design Director Engine** (revisa la pieza combinada con 14 criterios de crítica de dirección de arte — impacto visual, equilibrio, tensión visual, ritmo, punto focal, legibilidad, sensación premium...— veta si "parece plantilla automática" o si el título compite con el producto, y fuerza un patrón editorial distinto hasta aprobar), Layout Composer (orquesta los tres y renderiza sin cajas/tarjetas, con precio y contacto de marca reales — `brand-kit.json#contact`), umbral de calidad + reintento acotado en cada capa | `creative-engine/creative-lab/` | Node.js (CommonJS) puro. Reutiliza el Provider Manager de `creative-engine/` y `design-studio/scripts/render-html.js` sin cambios — independiente del proveedor de imágenes | No se despliega (CLI/script) | Activo — pipeline completo verificado extremo a extremo (concepto → patrón editorial → composición calculada y puntuada → revisión de dirección de arte → layout final sin cajas → precio/contacto reales); la campaña real del Ventilador Muvip aprueba con 92/100 (excelente) en Design Director Engine; primera entrada real (`sourceType: 'campana-propia'`) registrada en la Biblioteca de Referencias — la "memoria" del sistema ya está activa. **Objetivo principal (RT-09) sin resolver todavía**: la fotografía de la campaña Muvip fue aportada manualmente por el propietario y compuesta tal cual (`simulated.provider.js`, sin IA) — ningún proveedor real de generación de imágenes está conectado hoy, así que esto NO demuestra que el sistema genere fotografía de calidad equivalente por sí solo, ver `.claude/rax/DECISIONES.md` 2026-07-26 y `creative-engine/creative-lab/ARCHITECTURE.md` |
