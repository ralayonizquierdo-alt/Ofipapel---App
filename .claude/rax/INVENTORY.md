...
| Creative Lab | Módulo de investigación y perfeccionamiento de calidad visual dentro de Creative Engine — 9 bibliotecas atómicas, Biblioteca de Referencias (combina sin copiar, escalable a decenas de miles, con importador real desde campañas propias aprobadas — `reference-library/import-from-campaign.js`), Generador de Conceptos (8-12 por campaña) con filtro "Director de Arte Senior", Moodboard textual, Prompt Composer Cinematográfico (13 bloques, campaña no producto), Concept Score en dos capas, **Layout Intelligence** (calcula grid/jerarquía/tamaños relativos/márgenes/espacios en blanco/equilibrio y los puntúa ANTES de renderizar — sin coordenadas fijas, descarta y prueba otra estrategia si no supera el umbral), Layout Composer (orquesta Layout Intelligence y renderiza, con precio y contacto de marca reales — `brand-kit.json#contact`), umbral de calidad + reintento acotado | `creative-engine/creative-lab/` | Node.js (CommonJS) puro. Reutiliza el Provider Manager de `creative-engine/` y `design-studio/scripts/render-html.js` sin cambios — independiente del proveedor de imágenes | No se despliega (CLI/script) | Activo — pipeline completo verificado extremo a extremo (concepto → composición calculada y puntuada → layout final → precio/contacto reales); primera entrada real (`sourceType: 'campana-propia'`) registrada en la Biblioteca de Referencias — la "memoria" del sistema ya está activa. **Objetivo principal (RT-09) sin resolver todavía**: la fotografía de la campaña Muvip fue aportada manualmente por el propietario y compuesta tal cual (`simulated.provider.js`, sin IA) — ningún proveedor real de generación de imágenes está conectado hoy, así que esto NO demuestra que el sistema genere fotografía de calidad equivalente por sí solo, ver `.claude/rax/DECISIONES.md` 2026-07-26 y `creative-engine/creative-lab/ARCHITECTURE.md` |
