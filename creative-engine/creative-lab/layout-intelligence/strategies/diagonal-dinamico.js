...
function computePlan(grid, tierByElement, elementIds, stackOrder, artDirection) {
  ...
  const heroColStart = hero ? grid.columns - hero.colSpan : grid.columns;
  ...
  if (elementIds.includes('icons')) {
    ...
    elements.push({ elementId: 'icons', kind: 'icon-row', box: cellsToBox(grid, 0, Math.min(span.colSpan, textColSpan), cursorRow, span.rowSpan) });
  }
  ...
  return { elements, decorations: hero ? [{ type: 'diagonal-accent', splitX }] : [] };
}
