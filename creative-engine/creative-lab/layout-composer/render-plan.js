 ...
 * @param {{brand:object, product:object, copy:object, heroImagePath:string, width:number, height:number, textEmphasis:string, icons:object[], allowCard:boolean}} data
...
function buildHtmlFromPlan(planResult, data) {
  const { brand, product, copy, heroImagePath, width, height, textEmphasis, icons, allowCard } = data;
  ...
  if (byId.hero) layers.push(heroMarkup(byId.hero, heroImagePath, allowCard));
  ...
  if (byId.icons) layers.push(iconRowMarkup(byId.icons, icons, heroOnPhoto ? '#ffffff' : primary, width));
