...
function bloqueDeConsumibles(impresoras) {
  ...
  return [
    ...
    '- Esta lista dice qué consumible LE VALE a esa impresora, NO que lo tengamos en',
    '  tienda ni a qué precio. NUNCA digas "en stock", "disponibles" ni "tenemos"',
    '  apoyándote en esta lista: eso solo lo sabes por el bloque PRODUCTOS. Si ahí no hay',
    '  nada, dile la referencia que necesita y ofrécele consultar precio y disponibilidad.',
    '- Estas son las referencias del FABRICANTE. De casi todas hay además versión',
    '  genérica (o sea, compatible: lo mismo, pero de otra marca y más barato). Cuál de',
    '  las dos tenemos y a qué precio sale ÚNICAMENTE del bloque PRODUCTOS, donde cada',
    '  uno va etiquetado como ORIGINAL o GENÉRICO. Si ahí no hay ninguno marcado como',
    '  GENÉRICO, no des por hecho que lo tenemos: di que lo consultas.',
  ].join('\n');
}
