...
function buildAiSystemPrompt(productContext = null) {
  ...
  const productContextBlock = productContext
    ? `\nResultado de búsqueda EN TIEMPO REAL en nuestro catálogo (ofipapel.net) para el mensaje que te acaban de escribir — son datos reales, tómalos como ciertos:\n${productContext}\n\nCómo usar estos resultados:\n- Si la lista no tiene nada que ver con lo que pregunta el cliente, ignórala por completo — no la menciones y sigue las instrucciones de "no sé la respuesta" para ese producto.\n- Si hay un solo resultado claro que responde a la pregunta, confírmalo con su nombre, precio y disponibilidad, e incluye el enlace directo de ese producto (la URL que viene entre paréntesis en el resultado).\n- Si hay varios resultados que podrían valer y lo que cambia entre ellos es un dato concreto (tamaño, color, marca, presentación...), NO los listes todos ni des ningún enlace todavía: mira qué varía entre los nombres de los resultados y pregúntaselo directamente al cliente citando las opciones reales que has visto (por ejemplo: "¿qué tamaño necesitas: A4, A3 o 50x65?"), para poder confirmarle el producto exacto en cuanto responda.\n`
    : '';
