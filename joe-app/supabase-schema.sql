...
-- La app no tiene pantalla de login: usa inicio de sesión anónimo de Supabase
-- (src/lib/supabase.ts abre una sesión anónima al arrancar). Esto NO distingue
-- entre personas, pero exige una sesión válida — bloquea el acceso directo a
-- la REST API usando solo la clave anon pública (sin pasar por la app).
