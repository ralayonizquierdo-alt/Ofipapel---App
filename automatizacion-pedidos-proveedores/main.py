...
def main():
    ...
    balance_tolerancia_porcentaje = float(config.get("reglas", {}).get("balance_tolerancia_porcentaje", 0.0))
    ...
    df_consolidado, reporte = excel_logic.cruzar_y_determinar_ganador(
        ...
        balance_tolerancia_porcentaje=balance_tolerancia_porcentaje,
    )

    hoy = date.today().isoformat()

    # Inicializar Excel COM una sola vez para todos los proveedores
    xl = None
    usar_com = False
    try:
        ...
        print("AVISO: pywin32 no instalado, se generaran .xlsx en lugar de .xls")
    except Exception as e:
        print(f"AVISO: no se pudo iniciar Excel COM ({e}), se generaran .xlsx")
        ...
        try:
            ...
            if usar_com:
                ...
                try:
                    ...
                    print(f"XLS generado: {ruta_xls}")
                except Exception as e:
                    import traceback
                    print(f"ERROR convirtiendo a XLS: {e}")
                    traceback.print_exc()
            else:
                print(f"Excel generado (xlsx): {ruta_xlsx_tmp}")
